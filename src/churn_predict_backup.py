...
def check_data_quality(df):
    ...
    for col in numeric_cols:
        if col in ['SalesAmount', 'OrderQuantity', 'UnitPrice', 'ExtendedAmount', 'YearlyIncome']:
    ...
    if 'OrderDate' in df.columns:
        ...
        if date_range < 365:
            print(f"警告: 数据时间跨度较短，可能影响模型泛化能力")
        
        # 检查时间分布
        monthly_counts = df.groupby(df['OrderDate'].dt.to_period('M')).size()
        print(f"月度订单分布: 最小值={monthly_counts.min()}, 最大值={monthly_counts.max()}")
        
        if monthly_counts.std() / monthly_counts.mean() > 0.5:
            print(f"警告: 订单时间分布不均匀，可能存在季节性影响")
...
def feature_engineering(df, cutoff_date):
    ...
    static_df = historical_df[static_cols].drop_duplicates("CustomerKey").set_index("CustomerKey")
    print(f"静态特征数量: {len(static_cols) - 1}个")  # 减去CustomerKey
    
    # 计算客户生命周期特征
    static_df['days_since_first_purchase'] = (cutoff_date - static_df['DateFirstPurchase']).dt.days
    ...
    for i, customer_key in enumerate(static_df.index):
        ...
        if len(customer_data) == 0:
            ...
            features = {
                ...
                'customer_lifetime_days': static_df.loc[customer_key, 'days_since_first_purchase']
            }
        else:
            # 有购买记录的客户
            ...
            # 频率特征
            order_frequency = total_orders / (last_order_days / 365) if last_order_days > 0 else 0
            ...
            monetary_quantiles = historical_df.groupby('CustomerKey')['SalesAmount'].sum()
            m_33, m_67 = monetary_quantiles.quantile([0.33, 0.67])
            
            # 动态RFM评分
            r_score = 3 if rfm_recency <= r_33 else (2 if rfm_recency <= r_67 else 1)
            f_score = 3 if rfm_frequency >= f_67 else (2 if rfm_frequency >= f_33 else 1)
            ...
            recent_12m_data = customer_data[customer_data['OrderDate'] >= twelve_months_ago]
            ...
            if recent_12m_orders > 0:
                trend_6m_vs_12m_orders = (recent_6m_orders / 6) / (recent_12m_orders / 12)
                trend_6m_vs_12m_orders = min(trend_6m_vs_12m_orders, 5)  # 限制最大值
            else:
                trend_6m_vs_12m_orders = 0
                
            if recent_12m_amount > 0:
                trend_6m_vs_12m_amount = (recent_6m_amount / 6) / (recent_12m_amount / 12)
    ...
    # 对分类特征进行编码
    categorical_cols = ['Gender', 'EnglishEducation', 'EnglishOccupation', 'HouseOwnerFlag', 'CommuteDistance']
    print(f"编码分类特征: {categorical_cols}")
    for col in categorical_cols:
        if col in final_features.columns:
            le = LabelEncoder()
            final_features[col + '_encoded'] = le.fit_transform(final_features[col].astype(str))
...
def compare_models(results):
    ...
    comparison_df = pd.DataFrame({
        ...
        'Training Time': [results[name]['Training Time'] for name in results.keys()]
    })
    ...
    # 标准化评分 (0-1)
    comparison_df['AUC_Norm'] = (comparison_df['AUC'] - comparison_df['AUC'].min()) / (comparison_df['AUC'].max() - comparison_df['AUC'].min())
    ...
    comparison_df['F1_Norm'] = (comparison_df['F1-Score'] - comparison_df['F1-Score'].min()) / (comparison_df['F1-Score'].max() - comparison_df['F1-Score'].min())
    comparison_df['Time_Norm'] = 1 - (comparison_df['Training Time'] - comparison_df['Training Time'].min()) / (comparison_df['Training Time'].max() - comparison_df['Training Time'].min())
    ...
    # 选择建议
    best_composite = comparison_df.loc[comparison_df['Composite_Score'].idxmax(), 'Model']
    ...
    # AUC比较
    axes[0, 0].bar(comparison_df['Model'], comparison_df['AUC'], color=['#4C72B0', '#DD8452', '#55A868'])
    axes[0, 0].set_title('Model AUC Comparison', fontsize=14)
    axes[0, 0].set_ylabel('AUC Score')
    axes[0, 0].tick_params(axis='x', rotation=45)
    
    # 准确率比较
    axes[0, 1].bar(comparison_df['Model'], comparison_df['Accuracy'], color=['#4C72B0', '#DD8452', '#55A868'])
    axes[0, 1].set_title('Model Accuracy Comparison', fontsize=14)
    axes[0, 1].set_ylabel('Accuracy')
    axes[0, 1].tick_params(axis='x', rotation=45)
    
    # 精确率-召回率比较
    axes[1, 0].bar(comparison_df['Model'], comparison_df['Precision'], label='Precision', alpha=0.8)
    axes[1, 0].bar(comparison_df['Model'], comparison_df['Recall'], label='Recall', alpha=0.8)
    ...
    # 训练时间比较
    axes[1, 1].bar(comparison_df['Model'], comparison_df['Training Time'], color=['#4C72B0', '#DD8452', '#55A868'])
    ...
    if hasattr(model, 'feature_importances_'):
        ...
        for i, (feature, importance) in enumerate(zip(top_features, top_importances)):
            if 'rfm_recency' in feature:
                print(f"  • {feature} ({importance:.3f}): 客户最近购买时间，反映客户活跃度")
            elif 'total_orders' in feature:
                print(f"  • {feature} ({importance:.3f}): 客户历史订单总数，反映客户忠诚度")
            elif 'amount_cv' in feature or 'amount_std' in feature:
                print(f"  • {feature} ({importance:.3f}): 消费金额波动性，反映客户消费稳定性")
            elif 'rfm_score' in feature:
                print(f"  • {feature} ({importance:.3f}): RFM综合评分，反映客户综合价值")
            elif 'YearlyIncome' in feature:
                print(f"  • {feature} ({importance:.3f}): 年收入水平，反映客户购买力")
            elif 'recent_6m' in feature:
                print(f"  • {feature} ({importance:.3f}): 最近6个月行为，反映客户近期活跃度")
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(REPORT_DIR, f"{model_name.lower().replace(' ', '_')}_feature_importance.png"), dpi=120)
        plt.close()
        ...
        for i, (feature, coef) in enumerate(zip(top_features, top_coefficients)):
            if coef > 0:
                print(f"  • {feature} (+{coef:.3f}): 正相关，该特征值越高，流失风险越高")
            else:
        ...
        for i, v in enumerate(top_coefficients):
            ax.text(v + (0.01 if v >= 0 else -0.01), i, f"{v:.3f}", va="center", fontsize=10)
        
        plt.tight_layout()
        plt.savefig(os.path.join(REPORT_DIR, f"{model_name.lower().replace(' ', '_')}_coefficients.png"), dpi=120)
        plt.close()
    
    # 模型解释性总结
    print(f"\n=== 模型解释性总结 ===")
    if hasattr(model, 'feature_importances_'):
        print(f"✓ 特征重要性分析: 可以清晰识别影响客户流失的关键因素")
        print(f"✓ 决策路径分析: 可以解释模型的预测逻辑")
        print(f"✓ 业务可解释性: 高，便于业务理解和应用")
    elif hasattr(model, 'coef_'):
        print(f"✓ 特征系数分析: 可以量化特征对流失风险的影响方向")
        print(f"✓ 线性关系解释: 可以解释特征与目标变量的线性关系")
        print(f"✓ 业务可解释性: 中等，需要结合业务知识理解")
    else:
        print(f"⚠ 模型解释性: 较低，难以直接解释预测结果")
        print(f"建议: 考虑使用可解释性更强的模型或后处理技术")
...
def train_model_with_timeseries(X, y, test_size=0.2):
    ...
    # 评估候选模型
    model_results = evaluate_candidate_models(X_train_scaled, y_train, X_test_scaled, y_test)
    ...
    # 分析最佳模型的解释性
    analyze_model_interpretability(best_model, X.columns, X_test_scaled, y_test, best_model_name)
    ...
    print(f"\n=== 模型稳定性评估 ===")
    if auc_score > 0.8:
        print(f"✓ 模型性能优秀 (AUC > 0.8)")
    elif auc_score > 0.7:
        print(f"✓ 模型性能良好 (AUC > 0.7)")
    elif auc_score > 0.6:
        print(f"⚠ 模型性能一般 (AUC > 0.6)")
    else:
        print(f"✗ 模型性能较差 (AUC ≤ 0.6)")
    
    if precision > 0.7 and recall > 0.7:
        print(f"✓ 精确率和召回率平衡良好")
    elif precision > 0.8:
        print(f"⚠ 精确率较高但召回率较低，可能漏掉部分流失客户")
    elif recall > 0.8:
        print(f"⚠ 召回率较高但精确率较低，可能产生较多误报")
    else:
        print(f"✗ 精确率和召回率都需要改进")
...
def cross_validate_model(X, y, n_splits=5):
    ...
    cv_scores = cross_val_score(clf, scaler.fit_transform(X), y, cv=tscv, scoring='accuracy')
...
def plot_churn_distribution(label_df):
    ...
    label_df_copy["churn_str"] = label_df_copy["churn"].map({0: "Not Churn", 1: "Churn"})
    ...
    plt.title(f"Customer Churn Distribution (Churn Rate: {churn_rate:.1f}%)", fontsize=14)
...
def identify_high_risk_customers(clf, scaler, features_df, customer_keys, threshold=0.7):
    """识别高风险客户"""
    # 对所有客户进行预测
    ...
    print(f"  概率分布范围: {results_df['churn_probability'].min():.3f}-{results_df['churn_probability'].max():.3f}")
    ...
    for level in ['低风险', '中低风险', '中高风险', '高风险']:
        ...
        if len(level_data) > 0:
            ...
            print(f"  {level}: 平均概率 {avg_prob:.3f}, 范围 {min_prob:.3f}-{max_prob:.3f}")
    ...
    if len(high_risk) > 0:
        ...
        print(f"  概率分布集中度: {'高' if high_risk['churn_probability'].std() < 0.1 else '中等' if high_risk['churn_probability'].std() < 0.2 else '低'} (标准差: {high_risk['churn_probability'].std():.3f})")
    
    # 概率阈值选择建议
    print(f"\n=== 概率阈值选择建议 ===")
    print(f"业务阈值: 70% (基于业务成本和收益平衡)")
    print(f"精确率阈值: 85% (高精确率，减少误报)")
    print(f"召回率阈值: 60% (确保覆盖大部分流失客户)")
    
    # 概率应用策略
    print(f"\n=== 概率应用策略 ===")
    print(f"个性化营销: 根据概率值制定差异化策略")
    print(f"资源分配: 优先关注高概率客户")
    print(f"干预时机: 概率超过80%时立即干预")
...
def select_features(X, y, method='correlation', threshold=0.01):
    ...
    print(f"特征选择策略: 基于数据质量和业务重要性进行特征筛选")
    ...
    if method == 'correlation':
        ...
        # 显示高相关性特征
        high_corr_features = correlations[correlations > 0.1].sort_values(ascending=False)
        if len(high_corr_features) > 0:
            print(f"  高相关性特征 (|r| > 0.1):")
        ...
        high_mi_features = mi_df[mi_df['mi_score'] > 0.01].sort_values('mi_score', ascending=False)
        if len(high_mi_features) > 0:
            print(f"  高互信息特征 (MI > 0.01):")
    ...
    behavior_features = [f for f in selected_features if any(x in f.lower() for x in ['order', 'amount', 'frequency', 'trend'])]
    demographic_features = [f for f in selected_features if any(x in f.lower() for x in ['income', 'children', 'education', 'occupation', 'gender'])]
    other_features = [f for f in selected_features if f not in rfm_features + behavior_features + demographic_features]
    ...
    if len(selected_features) < len(X.columns) * 0.5:
        print(f"✓ 特征选择有效，减少了 {len(X.columns) - len(selected_features)} 个冗余特征")
    elif len(selected_features) > len(X.columns) * 0.9:
        print(f"⚠ 特征选择效果有限，建议调整阈值或使用其他方法")
    else:
        print(f"✓ 特征选择适中，平衡了特征数量和模型性能")
    ...
    for col in feature_names:
        if col in data.columns:
            ...
            if data[col].dtype == 'object' or data[col].nunique() < 10:
                ...
                # 数值特征使用直方图
                ax = sns.histplot(
                    data=data, x=col, bins=30, kde=True,
...
def plot_feature_vs_target(data, feature_names, target_col='churn', max_features=10):
    ...
    for col in feature_names:
        if col in data.columns and col != target_col:
            ...
            if data[col].dtype == 'object' or data[col].nunique() < 10:
                # 分类特征使用分组条形图
                ax = sns.countplot(data=data, x=col, hue=target_col, palette=["#4C72B0", "#DD8452"])
                ...
                # 数值特征使用箱线图
                ax = sns.boxplot(data=data, x=target_col, y=col, palette=["#4C72B0", "#DD8452"])
                plt.title(f"{col} vs {target_col}", fontsize=14)
...
def main():
    ...
    if total_days < (min_history_days + min_future_days):
        print(f"警告: 数据时间跨度不足 ({total_days}天 < {min_history_days + min_future_days}天)")
        # 使用80%的数据作为历史数据
    ...
    # 生成主要特征分布图
    main_features = [
        'total_orders', 'total_amount', 'avg_order_value', 'rfm_recency',
        'rfm_frequency', 'rfm_monetary', 'rfm_score', 'recent_6m_orders',
        'recent_6m_amount', 'trend_6m_vs_12m_orders', 'trend_6m_vs_12m_amount',
        'YearlyIncome', 'TotalChildren', 'NumberCarsOwned', 'is_new_customer'
    ]
    ...
    # 生成特征与目标变量关系图
    key_features = ['total_orders', 'rfm_recency', 'rfm_score', 'recent_6m_orders', 'YearlyIncome']
    available_key_features = [col for col in key_features if col in data.columns]
    plot_feature_vs_target(data, available_key_features, target_col='churn', max_features=8)
    ...
    # 12. 训练模型
    print("\n10. 训练模型...")
    clf, X_test, y_test, y_pred, y_score, scaler, comparison_df = train_model_with_timeseries(X_selected, y)
    ...
    # 14. 识别高风险客户
    print("\n12. 识别高风险客户...")
    all_results, high_risk = identify_high_risk_customers(clf, scaler, features[selected_features + ['CustomerKey']], customer_keys)
    
    # 15. 保存结果
    print("\n13. 保存结果...")
    # 保存所有客户的预测结果
    all_results.to_csv(os.path.join(REPORT_DIR, "all_customer_predictions.csv"), index=False)
    ...
    feature_selection_df.to_csv(os.path.join(REPORT_DIR, "selected_features.csv"), index=False)
    
    # 保存模型性能报告
    with open(os.path.join(REPORT_DIR, "model_performance.txt"), "w", encoding="utf-8") as f:
        ...
        f.write(f"交叉验证准确率: {cv_scores.mean():.4f} (+/- {cv_scores.std() * 2:.4f})\n")
        ...
        f.write(f"模型复杂度: {'中等' if 'Random Forest' in best_model_name else '较低' if 'Logistic' in best_model_name else '较高'}\n")
        f.write(f"特征解释性: {'优秀' if 'Random Forest' in best_model_name else '良好' if 'Logistic' in best_model_name else '一般'}\n\n")
        
        # 添加特征工程信息
        f.write("=== 特征工程信息 ===\n")
        f.write(f"特征工程策略: 严格时间分割，避免数据泄露\n")
        f.write(f"特征类型分布:\n")
        f.write(f"  • 静态特征: 人口统计、地理特征等\n")
        f.write(f"  • 行为特征: 购买频率、消费金额等\n")
        f.write(f"  • RFM特征: 最近购买时间、购买频率、消费金额\n")
        f.write(f"  • 趋势特征: 近期行为变化趋势\n")
        f.write(f"  • 生命周期特征: 客户生命周期天数\n")
        f.write(f"  • 编码特征: 分类特征编码\n\n")
        
        # 添加数据质量控制信息
        f.write("=== 数据质量控制 ===\n")
        f.write(f"数据泄露检测: 时间序列分割验证\n")
        f.write(f"异常值处理: IQR方法 + 业务规则\n")
        f.write(f"特征选择: 相关性分析 + 重要性排序\n")
        f.write(f"模型验证: 时间序列交叉验证\n\n")
        
        # 添加业务建议
        f.write("=== 业务建议 ===\n")
        f.write(f"• 建立客户流失预警监控体系\n")
        f.write(f"• 实施差异化客户服务策略\n")

...
def plot_rfm_clusters(rfm_clustered):
    ...
    centers = (
        rfm_clustered.groupby("Cluster")[["Recency", "Monetary"]].mean().reset_index()
    )
    for i, row in centers.iterrows():
        plt.scatter(
            row["Recency"],
            row["Monetary"],
            c=[palette[int(row["Cluster"])]],
            marker="X",
            s=180,
            edgecolor="black",
            linewidths=2,
    ...
    plt.title(
        "RFM Cluster Analysis (Recency vs Monetary)", fontsize=16, fontweight="bold"
    )
    ...
    centers2 = (
        rfm_clustered.groupby("Cluster")[["Frequency", "Monetary"]].mean().reset_index()
    )
    ...
    plt.title(
        "RFM Cluster Analysis (Frequency vs Monetary)", fontsize=16, fontweight="bold"
    )
...
def cluster_naming_and_advice(rfm_clustered):
    ...
    for _, row in summary.iterrows():
        ...
        if (
            ...
            and row["Monetary"] > summary["Monetary"].median()
        ):
            names[c] = "重要价值客户"
        elif (
            row["Recency"] < summary["Recency"].median()
    ...
    with open(out_dir + "rfm_cluster_naming_advice.txt", "w", encoding="utf-8") as f:
        for c, name in names.items():
            f.write(f"群体{c}：{name}\n")
            f.write(
                f"RFM均值：Recency={summary.loc[summary['Cluster'] == c, 'Recency'].values[0]:.1f}，Frequency={summary.loc[summary['Cluster'] == c, 'Frequency'].values[0]:.1f}，Monetary={summary.loc[summary['Cluster'] == c, 'Monetary'].values[0]:.1f}\n"
            )
...
def plot_rfm_radar(rfm_clustered):
    ...
    # 计算每个群体的RFM均值
    cluster_means = rfm_clustered.groupby("Cluster")[
...
def plot_customer_value_distribution(rfm_clustered):
    ...
    plt.tight_layout()
    plt.savefig(
        out_dir + "customer_value_distribution.png", dpi=200, bbox_inches="tight"
    )
    plt.close()
...
def analyze_product_preferences(df, clusters, engine):
    ...
    # 添加群体信息
    df_with_products = df_with_products.merge(
        clusters[["CustomerKey", "Cluster"]], on="CustomerKey"
    )

    # 按群体和产品类别统计
    preferences = (
        df_with_products.groupby(["Cluster", "EnglishProductCategoryName"])
        .agg({"SalesAmount": ["sum", "mean", "count"], "ProductKey": "nunique"})
        .reset_index()
    )
...
def analyze_seasonal_patterns(df, clusters):
    ...
    # 按群体和月份统计
    seasonal = (
        df.merge(clusters[["CustomerKey", "Cluster"]], on="CustomerKey")
...
def analyze_customer_lifecycle(df, clusters):
    ...
    lifecycle = (
        ...
        .agg({"OrderDate": ["min", "max", "count"], "SalesAmount": ["sum", "mean"]})
        .reset_index()
    )
    ...
    # 计算生命周期长度（天）
    lifecycle["LifecycleDays"] = (
        lifecycle["LastPurchase"] - lifecycle["FirstPurchase"]
    ).dt.days
...
def main():
    ...
    # 分析产品偏好
    _preferences = analyze_product_preferences(df, rfm_clustered, engine)

    # 分析季节性特征
    _seasonal = analyze_seasonal_patterns(df, rfm_clustered)

    # 分析客户生命周期
    _lifecycle = analyze_customer_lifecycle(df, rfm_clustered)

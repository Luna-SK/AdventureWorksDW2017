...
def feature_engineering(df):
    # 静态特征直接保留
    static_cols = [
        'CustomerKey', 'BirthDate', 'Gender', 'YearlyIncome', 'TotalChildren',
        'NumberChildrenAtHome', 'EnglishEducation', 'EnglishOccupation',
        'HouseOwnerFlag', 'NumberCarsOwned', 'DateFirstPurchase', 'CommuteDistance'
    ]
    ...
    # 消费金额波动率
    amount_std = recent_df.groupby('CustomerKey')['SalesAmount'].std().rename('amount_std_6m')
    amount_mean = recent_df.groupby('CustomerKey')['SalesAmount'].mean().rename('amount_mean_6m')
...
def generate_label(df):
    ...
    label_df['churn'] = label_df['CustomerKey'].apply(lambda x: 1 if x in inactive_customers else 0)
    return label_df
...
def plot_churn_distribution(label_df):
    ...
    ax = sns.countplot(x='churn_str', data=label_df, hue='churn_str', palette=['#4C72B0', '#DD8452'], legend=False, order=['Not Churn', 'Churn'])
    ax.set_xticks([0, 1])
...
def plot_feature_hist(features, feature_names):
    for col in feature_names:
        plt.figure(figsize=(5,4))
        ax = sns.histplot(features[col], bins=30, kde=True, color='#4C72B0', edgecolor='black', alpha=0.8)
...
def plot_feature_importance(clf, feature_names):
    ...
    ax = sns.barplot(x=importances[indices], y=[feature_names[i] for i in indices], color='#4C72B0', orient='h')
...
def main():
    ...
    X = data.drop(['CustomerKey', 'churn', 'BirthDate', 'DateFirstPurchase'], axis=1, errors='ignore')
    y = data['churn']

    # 可视化主要特征分布
    main_features = [col for col in ['freq_6m', 'amount_cv_6m', 'YearlyIncome', 'TotalChildren', 'NumberCarsOwned'] if col in X.columns]
    ...
    high_risk[['CustomerKey']].to_csv(os.path.join(REPORT_DIR, 'high_risk_customers.csv'), index=False)

...
def build_baskets(df, season):
    ...
    # 构建订单-产品二元表
    basket = (
        season_df.groupby(["SalesOrderNumber", "EnglishProductName"])["OrderQuantity"]
        .sum()
        .unstack()
        .fillna(0)
    )
...
def plot_top_rules(rules, season, topn=10):
    ...
    plt.savefig(
        os.path.join(REPORT_DIR, f"{season}_top_rules.png"),
        dpi=120,
        bbox_inches="tight",
    )
    plt.close()
...
def plot_freq_itemsets_heatmap(basket, season):
    ...
    plt.savefig(
        os.path.join(REPORT_DIR, f"{season}_heatmap.png"), dpi=120, bbox_inches="tight"
    )
    plt.close()
...
def save_top_rules_text(rules, season, topn=3):
    ...
    for i, row in rules.head(topn).iterrows():
        ...
        lines.append(
            f"规则{i + 1}: 如果购买[{ant}]，则很可能购买[{cons}]。\n"
            f"  - 支持度: {row['support']:.3f}，置信度: {row['confidence']:.3f}，提升度: {row['lift']:.3f}\n"
        )
        # 业务建议举例
        lines.append(
            f"  - 建议：可将[{ant}]与[{cons}]进行捆绑促销或货架邻近陈列，提升联动销售。\n"
        )
    with open(
        os.path.join(REPORT_DIR, f"{season}_top_rules.txt"), "w", encoding="utf-8"
    ) as f:
        f.writelines(lines)
...
def plot_rules_network(rules, season, min_conf=0.5):
    ...
    plt.savefig(
        os.path.join(REPORT_DIR, f"{season}_rules_network.png"),
        dpi=120,
        bbox_inches="tight",
    )
    plt.close()
...
def plot_seasonal_comparison(seasonal_rules):
    ...
    metrics = {
        ...
        "Avg Confidence": [
            rules["confidence"].mean() for rules in seasonal_rules.values()
        ],
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(REPORT_DIR, "seasonal_rules_comparison.png"),
        dpi=120,
        bbox_inches="tight",
    )
    plt.close()
